...
namespace Voice
{
	...
	{
	public:
		// Читает файл и разбирает его в поля по месту: при отказе часть полей
		// может оказаться уже заполненной, но это не важно - без настроек
		// адаптер работать не станет, и решает об этом вызывающий.
		//
		// Отказ здесь строгий нарочно: неверный тип у ключа - отказ целиком.
		// Прежде поля читались в момент использования, и кривой ключ ронял
		// игру из потока опроса либо давал «ответ не разобран» на каждую
		// реплику. Единственное послабление - выключенные модели: у них
		// читаются только имя и признак, остальное не проверяется.
		static bool Load();
