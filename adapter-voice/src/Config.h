...
namespace Voice
{
	...
	struct Model
	{
		...
		// Разовый секрет этой сессии. Его выдумывает адаптер, отдаёт службе при
		// запуске и потом присылает в заголовке каждого запроса. Служба, которая
		// его проверяет, не станет разговаривать с чужой программой; служба,
		// которая о нём не знает, просто его не заметит.
		std::string token;
	};
