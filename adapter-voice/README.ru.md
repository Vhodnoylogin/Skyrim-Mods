...
Полный контракт листка: [contract/envoy-voice-model.ru.md](contract/envoy-voice-model.ru.md).
...
переводится наравне со всем прочим в Envoy — строки в коде не написаны, там стоят ключи,
а тексты лежат в `Interface\Translations\EnvoyVoiceAdapter_<язык>.txt`. Читает их
заголовок `envoy-loc.h` из SDK моста: адаптер — отдельная библиотека и пишет свои первые
...
Язык берётся из ключа `language` в `envoy-voice.json`; `auto` — язык самой игры.
...
`Envoy Framework - SDK`, и путь к нему задан в `config/build.json` ключом `deploy.sdk`. Это не неудобство, а проверка — собранный против
