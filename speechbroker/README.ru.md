...
- **Адаптер** владеет микрофоном, и владеет им внутри игры: захват звука, тишина и границы
  фраз - его дело, и делается это в его же процессе, а не за сокетом. Внутрь игры адаптер
  говорит вызовом функции через контракт моста. Ни одной модели по имени он не знает.
- **Мод-модель** - это **прослойка**: такой же плагин SKSE, как и двое остальных. Одной
  стороной она говорит на контракте адаптера, другой - либо сама **является** моделью, либо
  **поднимает** её отдельным процессом, либо **присоединяется** к уже работающей: здесь или
  на другой машине. Что именно из четырёх - она объявляет, и род можно запретить в настройках
  адаптера.

Отсюда два свойства сразу. **Игроку нечего запускать руками** — он ставит моды, а игра
поднимает всё сама. И **смена модели не требует ни новой сборки, ни правки настроек**:
человек ставит другой мод, а микрофон остаётся один на всех.
