# Laying the SpeechBroker bridge out into mods\. Every path and name is in
...
# and so does any other mod that wants to talk to SpeechBroker.
#
...
if (Test-Path -LiteralPath $dll) {
    New-Item -ItemType Directory -Force (Join-Path $mod 'SKSE\Plugins\speechbroker') | Out-Null
    Copy-Item -LiteralPath $dll -Destination (Join-Path $mod 'SKSE\Plugins') -Force
    Copy-Item -LiteralPath (Expand-Path $d.defaultConfig) `
              -Destination (Join-Path $mod 'SKSE\Plugins\speechbroker\speechbroker.json') -Force
} else {
...
# here into what the game reads: Interface\Translations\SpeechBroker_<language>.txt,
...
$sdkMeta = @(
    ...
    'category="0,"'
    'installationFile='
    'notes=Speech Broker - the package for mod authors: the contract, the Papyrus declarations, the script sources and the string tables. The game does not need it; keep it disabled in the profiles.'
    ''
    '[installedFiles]'
    'size=0'
)
...
$meta = @(
    ...
    'category="0,"'
    'installationFile='
    'notes=Speech Broker - the bridge. The contract for adapters and listeners is in the SDK package.'
    ''
    '[installedFiles]'
    'size=0'
)
...
if (-not $NoIndex) {
    & $d.indexScript -Owner $d.indexOwner -Mods $copied -Note "Speech Broker deploy $($d.version)"
}
''
