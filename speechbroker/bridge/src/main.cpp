// Speech Broker - the bridge between Skyrim and outside models.
//
...
namespace
{
	...
	const char* MessageName(std::uint32_t a_type)
	{
		...
		default:                                      return SpeechBroker::Loc::Get("$SPEECHBROKER_WORD_UNKNOWN");
		}
	}
	...
	void OnMessage(SKSE::MessagingInterface::Message* a_message)
	{
		...
		SpeechBroker::Log::Info("$SPEECHBROKER_LOG_SKSE_MESSAGE", MessageName(a_message->type), a_message->type);
		...
		if (a_message->type == SKSE::MessagingInterface::kDataLoaded) {
			SpeechBroker::GameLoadWatch::Install();
		}
	}
}
...
extern "C" __declspec(dllexport) bool SKSEAPI SKSEPlugin_Load(const SKSE::LoadInterface* a_skse)
{
	...
	SpeechBroker::Log::Init(settings.logLevel, logFile, settings.logMaxSizeKb);
	SpeechBroker::Log::SetShowSpeech(settings.logSpeechText);
	...
	SpeechBroker::Loc::Load(kTranslations, ResolveLanguage(settings.language));
	SpeechBroker::Settings::Report();
	...
	SpeechBroker::Log::Info("$SPEECHBROKER_LOG_PLUGIN_LOADED", PLUGIN_NAME, PLUGIN_VERSION,
		SpeechBrokerAPI::kInterfaceVersion);
	SpeechBroker::Log::Info("$SPEECHBROKER_LOG_CONFIG_SOURCE",
		SpeechBroker::Loc::Get(SpeechBroker::Config::Describe(config.Source())), config.Path().string());

	if (!config.Error().empty()) {
		SpeechBroker::Log::Warn("$SPEECHBROKER_LOG_CONFIG_ERROR", config.Error());
	}

	if (auto* papyrus = SKSE::GetPapyrusInterface()) {
		papyrus->Register(SpeechBroker::PapyrusApi::Register);
	}
