...
namespace SpeechBroker
{
		...
		const BidRecord& a_top) const
	{
		...
		const char* stranger = a_top.phrase.empty()
		                           ? Loc::Get("$SPEECHBROKER_REASON_WINNER_UNKNOWN_COMMAND")
		                           : Loc::Get("$SPEECHBROKER_REASON_OTHER_COMMAND");
		...
		for (const auto& bid : a_survivors) {
			...
			if (!same) {
				...
				result.denied[bid.ns] = Loc::Get("$SPEECHBROKER_REASON_LOST_AND_GREEDY");
			} else {
	...
	std::vector<BidRecord> Auction::Survivors(Result& a_result) const
	{
		...
		for (const auto& bid : _utterance.bids) {
			if (bid.confidence >= Settings::Get().MinConfidence(bid.costClass)) {
				...
				a_result.denied[bid.ns] = Loc::Get("$SPEECHBROKER_REASON_BELOW_THRESHOLD");
			}
		}
		return survivors;
	}
	...
	Auction::Result Auction::BreakTie(const std::vector<BidRecord>& a_survivors, float a_need) const
	{
		...
		if (best != Settings::kNoPriority && count == 1) {
			return Exclusive(chosen->ns, Loc::Get("$SPEECHBROKER_REASON_PRIORITY"),
				a_survivors);
		}
		...
		if (!SameCommand(tied)) {
			result.reason = Loc::Get("$SPEECHBROKER_REASON_AMBIGUOUS");
			for (const auto& bid : a_survivors) {
		...
		if (Settings::Get().sharedWinsTie) {
			...
			if (!shared.winners.empty()) {
				shared.reason = Loc::Get("$SPEECHBROKER_REASON_SHARED_TAKES_TIE");
				return shared;
			}
		}

		result.reason = Loc::Get("$SPEECHBROKER_REASON_ALL_GREEDY");
		for (const auto& bid : a_survivors) {
			result.denied[bid.ns] = result.reason;
		}
		return result;
	...
	Auction::Result Auction::Decide() const
	{
		...
		if (_utterance.bids.empty()) {
			result.reason = Loc::Get("$SPEECHBROKER_REASON_NO_BIDS");
			return result;
		}
		...
		if (survivors.size() > 1 && top.confidence - survivors[1].confidence < need) {
			...
			verdict = Exclusive(top.ns, Loc::Get("$SPEECHBROKER_REASON_GREEDY_WON"), survivors);
		} else {
			verdict = Share(survivors, top);
			verdict.reason = verdict.winners.size() > 1
			                     ? Loc::Get("$SPEECHBROKER_REASON_SHARED_WON")
			                     : Loc::Get("$SPEECHBROKER_REASON_SHARED_ALONE");
		}
	...
	void Auctioneer::Receive(std::int32_t a_id)
	{
		...
		// SpeechBroker_Speech_World event - holding counted the risk by one room while the
		...
		if (!verdict.hold) {
			if (verdict.audience > 0) {
				Log::Info("$SPEECHBROKER_LOG_HANDED_AT_ONCE", a_id, verdict.reason);
			}
			Offer(a_id);
			return;
		}
		...
		UtteranceStore::Get().Update(a_id, item);
		Log::Info("$SPEECHBROKER_LOG_HELD", a_id, verdict.reason);
		...
		if (verdict.ceilingMs > 0) {
			Scheduler::Get().After(std::chrono::milliseconds(verdict.ceilingMs), [a_id]() {
				MainThread::Post([a_id]() {
					Auctioneer::Get().Release(a_id, Loc::Get("$SPEECHBROKER_REASON_CEILING"));
				});
			});
		}
	}
	...
	void Auctioneer::Release(std::int32_t a_id, const std::string& a_why)
	{
		...
		item.held = false;
		UtteranceStore::Get().Update(a_id, item);
		Log::Info("$SPEECHBROKER_LOG_LET_GO", a_id, a_why);
		Offer(a_id);
	}
	...
	void Auctioneer::Supersede(std::int32_t a_newId, const std::vector<std::int32_t>& a_older)
	{
		for (const auto older : a_older) {
			...
			if (wasHeld) {
				...
				Log::Info("$SPEECHBROKER_LOG_DROPPED",
					older, a_newId);
				continue;
			}
			...
			if (!item.winners.empty()) {
				...
				Log::Warn("$SPEECHBROKER_LOG_REVOKED",
					older, who, a_newId);
				Events::Send("SpeechBroker_Revoked", "", static_cast<float>(older));
			}
		}
	}
	...
	void Auctioneer::Offer(std::int32_t a_id)
	{
		...
		// SpeechBroker_Speech_Any by calling GetTopic. The text is always taken from the
	...
	void Auctioneer::Settle(std::int32_t a_id)
	{
		...
		if (outcome.empty()) {
			outcome = Loc::Get("$SPEECHBROKER_WORD_NOBODY");
		}
		...
		// for everybody - but for their condition: SpeechBroker_Award stays silent when
		// nobody won, SpeechBroker_Denied when nobody was refused, and SpeechBroker_Settled always
		// sounds.
		if (!result.winners.empty()) {
			Events::Send("SpeechBroker_Award", "", static_cast<float>(a_id));
		}
		if (!result.denied.empty()) {
			Events::Send("SpeechBroker_Denied", "", static_cast<float>(a_id));
		}
		Events::Send("SpeechBroker_Settled", "", static_cast<float>(a_id));
	}
}
