...
namespace SpeechBroker
{
	...
	// seize up for good.
	class AdapterHost final : public SpeechBrokerAPI::ISpeechBroker
	{
	public:
		static AdapterHost& Get();

		std::uint32_t Version() const override { return SpeechBrokerAPI::kInterfaceVersion; }

		bool         Register(const SpeechBrokerAPI::AdapterInfo& a_info, SpeechBrokerAPI::JobCallback a_onJob, void* a_user) override;
		void         Unregister(const char* a_id) override;
		std::int32_t PushUtterance(const char* a_adapterId, const SpeechBrokerAPI::UtteranceIn& a_utterance) override;
	...
	private:
		...
		struct Entry
		{
			...
			SpeechBrokerAPI::JobCallback    onJob{ nullptr };
			void*                    user{ nullptr };
		...
		// the job itself: the pointers inside SpeechBrokerAPI::Job live only for the length
