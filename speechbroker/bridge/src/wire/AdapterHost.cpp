...
namespace SpeechBroker
{
	...
	bool AdapterHost::Register(const SpeechBrokerAPI::AdapterInfo& a_info, SpeechBrokerAPI::JobCallback a_onJob, void* a_user)
	{
		...
		if (a_info.contract < 1 || a_info.contract > SpeechBrokerAPI::kInterfaceVersion) {
			Log::Error("$SPEECHBROKER_LOG_ADAPTER_VERSION_REFUSED",
				a_info.id, a_info.contract, SpeechBrokerAPI::kInterfaceVersion);
			return false;
		}
		...
		Log::Info("$SPEECHBROKER_LOG_ADAPTER_REGISTERED",
	...
	void AdapterHost::Unregister(const char* a_id)
	{
		...
		Dispatch(pending);
		Log::Info("$SPEECHBROKER_LOG_ADAPTER_LEFT", a_id);
	}
	...
	std::vector<AdapterHost::Outgoing> AdapterHost::RecomputeSources()
	{
		...
		for (auto& entry : _adapters) {
			...
			out.kind = SpeechBrokerAPI::kJobListen;
			out.active = active;
			out.text = active
			                  ? fmt::format(fmt::runtime(Loc::Get("$SPEECHBROKER_ADAPTER_MADE_SOURCE")), role)
			                  : Loc::Get("$SPEECHBROKER_ADAPTER_OTHER_SOURCE");
			pending.push_back(std::move(out));

			Log::Info("$SPEECHBROKER_LOG_ADAPTER_ROLE", entry.first,
				Loc::Get(active ? "$SPEECHBROKER_WORD_SOURCE" : "$SPEECHBROKER_WORD_IN_RESERVE"));
		}

		return pending;
	}

	std::int32_t AdapterHost::PushUtterance(const char* a_adapterId, const SpeechBrokerAPI::UtteranceIn& a_in)
	{
		Utterance utterance;
		...
		if (a_in.refinesId != 0) {
			...
			if (Log::ShowSpeech()) {
				Log::Info("$SPEECHBROKER_LOG_REFINED_TEXT",
					a_in.refinesId, Safe(a_adapterId), utterance.engine, utterance.text);
			} else {
				Log::Info("$SPEECHBROKER_LOG_REFINED",
					a_in.refinesId, Safe(a_adapterId), utterance.engine);
				Log::Debug("$SPEECHBROKER_LOG_UTTERANCE_TEXT", a_in.refinesId, utterance.text);
			}
		...
		if (Log::ShowSpeech()) {
			Log::Info("$SPEECHBROKER_LOG_UTTERANCE_FROM_TEXT", id, Safe(a_adapterId), Safe(a_in.text));
		} else {
			Log::Info("$SPEECHBROKER_LOG_UTTERANCE_FROM", id, Safe(a_adapterId));
			Log::Debug("$SPEECHBROKER_LOG_UTTERANCE_TEXT", id, Safe(a_in.text));
		}
	...
	void AdapterHost::SendVocabulary(const std::vector<std::string>& a_phrases)
	{
		std::vector<Outgoing> pending;
		{
			...
			for (auto& entry : _adapters) {
				...
				out.onJob = entry.second.onJob;
				out.user = entry.second.user;
				out.kind = SpeechBrokerAPI::kJobVocabulary;
		...
		std::int32_t a_priority)
	{
		...
		std::string            target;
		{
			...
			if (source == _sources.end()) {
				Log::Warn("$SPEECHBROKER_LOG_NO_TTS");
				return 0;
			}
			...
			out.kind = SpeechBrokerAPI::kJobSpeak;
			out.text = a_text;
			out.service = a_voice;
		...
		Log::Info("$SPEECHBROKER_LOG_SPEECH_SENT", speechId, target, a_text);
		return speechId;
	}
	...
	void AdapterHost::SendStop(std::int32_t a_speechId)
	{
		std::vector<Outgoing> pending;
		{
			...
			for (auto& entry : _adapters) {
				...
				out.onJob = entry.second.onJob;
				out.user = entry.second.user;
				out.kind = SpeechBrokerAPI::kJobStop;
	...
	std::int32_t AdapterHost::SendAsk(const std::string& a_service, const std::string& a_payload)
	{
		...
		std::string            target;
		{
			...
			if (source == _sources.end()) {
				Log::Warn("$SPEECHBROKER_LOG_NO_SERVICE", a_service);
				return 0;
			}
			auto adapter = _adapters.find(source->second);
			if (adapter == _adapters.end()) {
			...
			out.onJob = adapter->second.onJob;
			out.user = adapter->second.user;
			out.kind = SpeechBrokerAPI::kJobAsk;
		...
		Log::Info("$SPEECHBROKER_LOG_REQUEST_SENT", requestId, a_service, target);
		return requestId;
	}
		...
		const char* a_payload)
	{
		...
		Log::Info("$SPEECHBROKER_LOG_ANSWER", a_requestId, Safe(a_adapterId),
			Loc::Get(a_ok ? "$SPEECHBROKER_WORD_SUCCESS" : "$SPEECHBROKER_WORD_FAILURE"));
		...
		MainThread::Post([a_requestId]() {
			Events::Send("SpeechBroker_Answer", "", static_cast<float>(a_requestId));
		});
	}
		...
		bool a_interrupted)
	{
		...
		Log::Info("$SPEECHBROKER_LOG_SPEECH_DONE", a_speechId, Safe(a_adapterId), how);

		MainThread::Post([a_speechId]() {
			Events::Send("SpeechBroker_SpeechDone", "", static_cast<float>(a_speechId));
		});
	}
}
