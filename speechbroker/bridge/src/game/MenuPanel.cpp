...
namespace SpeechBroker
{
	namespace
	{
		...
		{
			ImGuiMCP::Text(Loc::Get("$SPEECHBROKER_LOG_TITLE"));
			ImGuiMCP::Separator();

			ImGuiMCP::TextUnformatted(Loc::Get("$SPEECHBROKER_LOG_LEVEL"));

			int chosen = IndexOf(Log::Level());
			for (int i = 0; i < static_cast<int>(std::size(kLevels)); ++i) {
				if (ImGuiMCP::RadioButton(kLevels[i], &chosen, i)) {
					Log::SetLevel(kLevels[i]);
					Log::Info("$SPEECHBROKER_LOG_MENU_LEVEL", kLevels[i]);
				}
			...
			if (ImGuiMCP::Checkbox(Loc::Get("$SPEECHBROKER_LOG_SPEECH"), &speech)) {
				Log::SetShowSpeech(speech);
				Log::Info("$SPEECHBROKER_LOG_MENU_SPEECH",
					Loc::Get(speech ? "$SPEECHBROKER_WORD_ON" : "$SPEECHBROKER_WORD_OFF"));
			}
			ImGuiMCP::TextUnformatted(Loc::Get("$SPEECHBROKER_LOG_SPEECH_HELP"));

			ImGuiMCP::Separator();
			ImGuiMCP::TextUnformatted(Loc::Get("$SPEECHBROKER_LOG_SESSION_HELP"));
		}
	}
	...
	void MenuPanel::Install()
	{
		if (!SKSEMenuFramework::IsInstalled()) {
			Log::Info("$SPEECHBROKER_LOG_MENU_ABSENT");
			return;
		}
		...
		SKSEMenuFramework::SetSection("SpeechBroker");
		SKSEMenuFramework::AddSectionItem(Loc::Get("$SPEECHBROKER_MENU_LOG"), RenderLog);
		Log::Info("$SPEECHBROKER_LOG_MENU_ADDED");
	}
}
