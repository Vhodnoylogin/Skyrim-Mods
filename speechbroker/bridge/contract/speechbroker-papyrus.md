# Speech Broker - the in-game API

*In Russian: [speechbroker-papyrus.ru.md](speechbroker-papyrus.ru.md). English is the source language; the other is a translation.*

Contract version **3** - the same number `SpeechBroker.GetInterfaceVersion()` returns.
The declarations are in `SpeechBroker.psc`.
...
| `BID`       | `SpeechBroker.Bid(...)` - or silence | the mod |
| `AWARD`     | the bridge announces the winners | the bridge |
| `SETTLE`    | `SpeechBroker.Done(...)` - a report on how it ended | the mod |
...
|---|---|---|---|
| `SpeechBroker_Speech_Dialogue` | the dialogue window is open | the utterance | `GetText`, `GetVocabularyScore` |
| `SpeechBroker_Speech_Menu`     | another menu is open, the game is paused | the utterance | the same |
| `SpeechBroker_Speech_Combat`   | the player is in combat | the utterance | the same |
| `SpeechBroker_Speech_World`    | none of the above | the utterance | the same |
| `SpeechBroker_Speech_Channel`  | an explicit channel fired | the utterance | `GetChannel` |
| `SpeechBroker_Speech_Any`      | every utterance whatever the topic - for observers | the utterance | `GetTopic` |
| `SpeechBroker_Award`           | the lot is played out, there are winners | the utterance | `IsWinner`, `GetWinners` |
| `SpeechBroker_Denied`          | the lot is played out, somebody was refused | the utterance | `GetDenyReason` |
| `SpeechBroker_Settled`         | the lot is played out, whatever the outcome | the utterance | `GetOutcome` |
| `SpeechBroker_Revoked`         | an utterance already handed over was absorbed by a longer one | the utterance | the winner undoes what it did, if it can |
| `SpeechBroker_Ready`           | the bridge is ready, the game is loaded - participants declare themselves again | the contract version | `Subscribe`, `RegisterVocabulary`, `Declare` |
| `SpeechBroker_Ping`            | the self-test of delivery; the carrier quest answers | the token | `Pong` |
| `SpeechBroker_Answer`          | a model answered an `Ask` | the request | `GetAnswer` |
| `SpeechBroker_SpeechDone`      | speaking finished | the speech | `GetSpeechResult` |
...
reach the subscribers of `SpeechBroker_Speech_Dialogue` - that is a cut made by the fact of the state of
...
There is one `SpeechBroker_Award`, one `SpeechBroker_Denied` and one `SpeechBroker_Settled` per outcome rather than per
recipient: there is no name in the event, and each participant asks `IsWinner` or `GetDenyReason`
for itself. The three names are kept for their condition: `SpeechBroker_Award` stays silent when nobody
won, `SpeechBroker_Denied` when nobody was refused, and `SpeechBroker_Settled` always sounds.
...
you the unfinished thing earlier, but obliges you to listen for `SpeechBroker_Revoked` and to be able to
undo what you did.
...
in `Interface\Translations\Speech Broker*_<language>.txt` - Skyrim's own format, UTF-16LE, one
`$KEY<tab>text` line each. A mod puts its own file next to ours, names it
`Speech Broker<Something>_<language>.txt`, and its keys appear in the table.

The engine already resolves a `$`-prefixed string by itself when the whole string is shown as it
is, so `Debug.MessageBox("$MY_KEY")` works without any help. `SpeechBroker.Translate` is for the two cases
...
Function Register()
    ...
    SpeechBroker.Declare(NS, 1, false)   ; a spell is expensive and irreversible
...
2. **Do not compute for long in a handler.** The bid window is measured in tens of milliseconds.
   Every heavy check belongs after `SpeechBroker_Award`.
