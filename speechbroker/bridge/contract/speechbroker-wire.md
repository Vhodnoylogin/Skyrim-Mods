# Speech Broker - the adapter contract (the first edition, over HTTP)
...
> `speechbroker-adapter.h`. The document is kept because the rules in it - who calls whom, exclusivity per
...
**Speech Broker starts nothing.** It knows no external program, keeps no addresses for them and cannot talk
...
sides: its model and this contract. There can be as many adapters as you like, and Speech Broker learns
about them only at the moment they come and register themselves.

    model ──► adapter ──► Speech Broker ──► an event ──► a subscriber mod
    model ◄── adapter ◄── Speech Broker ◄── a request ◄── a subscriber mod

## The life cycle

1. The game starts and the Speech Broker plugin raises the server.
2. Speech Broker starts the programs **listed in its settings**. It does not know what those programs are -
...
Point 6 matters: **the connection IS the sign that somebody needs the model.** While Speech Broker is
...
again no less often than `adapters.registrationTtlSec`. If it disappears, Speech Broker counts its
...
## What the adapter puts into Speech Broker
...
by **Speech Broker** and not by the adapter: the number has to fit into a Papyrus integer and to be one and
...
## What Speech Broker hands to the adapter
...
1. **Speech Broker knows no models.** There must not be a single name of a program, a port or a model format
   in its code. All of that lives in the adapter and in the settings.
2. **The content of `payload` is opaque.** Speech Broker does not parse it - otherwise every new model would
