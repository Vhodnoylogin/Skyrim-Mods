...
Сердце Speech Broker: плагин SKSE, который принимает распознанную речь от адаптеров, выбирает ей тему,
...
Что такое Speech Broker целиком и как части соотносятся — в [описании модуля](../README.ru.md).
...
остальное мод забирает функциями `SpeechBroker.*` по этому номеру. Ответить из события нельзя,
...
Сторожит правило цель `speechbroker-host` из `tests/`: она собирает ядро **без**
...
`Interface\Translations\Speech Broker*_<язык>.txt`, штатного формата Skyrim. Таблица пишется
...
Все языки модуля едут внутри самого мода — и исходный, и переводы: перевод принадлежит
модулю, а не ставится рядом отдельным модом. Добавить язык — значит положить файл
в `localization\` и разложить мод заново; регистрировать его негде, движок читает
все файлы `Speech Broker*_<язык>.txt`, какие найдёт.
...
Подписчик добирается до той же таблицы через `SpeechBroker.Translate`. Она нужна там, где движок
...
## Прогон аукциона без игры
    ...
    speechbroker-host.exe --scenario tests\scenarios\default.json --report прогон.txt
    speechbroker-host.exe --config %TEMP%\priority.json    с заданным порядком участников

Настройки хост берёт из `speechbroker-host.json` рядом с собой, а `--config` подменяет
файл. `tests/speechbroker-host.json` — заготовка с непустым `auction.priority`: на ней
...
## Раскладка
    ...
    papyrus/    SpeechBroker.psc - объявления для слушателей, и скрипт квеста-носителя
    esp/        квест-носитель
...
Отдельная поставка `Speech Broker - SDK`, которая раскладывается рядом с модом:

| Файл | Кому |
|---|---|
| `cpp/speechbroker-adapter.h` | авторам адаптеров: C-ABI, версия контракта, структуры реплик |
| `cpp/speechbroker-abi.h` | авторам поставщиков состояния |
| `cpp/speechbroker-loc.h` | им же: чтение файлов перевода, заголовком, без сборки |
| `papyrus/SpeechBroker.psc` | авторам подписчиков: объявления Papyrus |
| `docs/speechbroker-papyrus.md` | им же: разбор каждого вызова и события |
| `docs/speechbroker-keys.md` | пространство ключей состояния |
| `localization/speechbroker.*.txt` | переводчикам: исходные таблицы моста |
| `tools/build-localization.py` | всем: превращение таблицы в то, что читает игра |

Версия контракта берётся **из двоичного файла** (`SpeechBrokerAPI::kInterfaceVersion`), а не из
