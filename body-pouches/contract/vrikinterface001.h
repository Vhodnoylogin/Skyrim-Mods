...
namespace vrikPluginApi {
	...
	public:
		...
		//Check Build number 80700 or higher before calling these functions below

		//Holster functions. 
		virtual int GetHolsterSlotInReach(bool secondaryHand) = 0;
		virtual bool CanBeHolstered(bool secondaryHand) = 0;
		virtual bool HolsterWeapon(bool secondaryHand) = 0;
		virtual bool UnHolsterWeapon(bool secondaryHand) = 0;

		//Slot state functions
		virtual bool VrikGetSlotDisplayed(int slotNumber) = 0;
		virtual void VrikSetSlotWeaponType(int slotNumber, TESForm * weapon) = 0;
		virtual void VrikSetSlotEquipped(int slotNumber, int hand) = 0;
		virtual void VrikSetSlotForArt(int slotNumber, BGSArtObject * artObject) = 0;

		//Slot suspension system functions. Suspended slots remain detectable, but VRIK ignores them for activation. Suspension is runtime-only.
		//HolsterAttemptCallback (works for all slots, not only suspended ones) requires a return value of true for allowing normal VRIK action. False consumes the event, no VRIK action. 
		//If slot is suspended, regardless of callback return values, VRIK doesn't do anything.
