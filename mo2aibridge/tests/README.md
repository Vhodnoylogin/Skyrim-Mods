...
`plugin-mo2aibridge\`, а не её часть. Сам плагин они находят рядом, по наличию `__init__.py`,
а не по совпадению имён — имена как раз намеренно разные.

Грузится он **по пути**, а не обычным импортом: `plugin-mo2aibridge` из-за дефиса не может быть
именем модуля Python. Имя, под которым плагин известен MO2, проверки спрашивают у него самого —
строка `PLUGIN_ID` в `__init__.py`; из неё же берётся имя файла с токеном.
