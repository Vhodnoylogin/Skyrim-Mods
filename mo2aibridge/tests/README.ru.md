...
Все девять наборов подряд. Наборы, которым нужна MO2, пропускаются с внятным словом, если она
не запущена, — пропуск не считается сбоем.

Слова самих проверок локализованы так же, как и слова плагина: каталог лежит
в `locale\<язык>\`, а `MO2AIBRIDGE_LANG=ru` прогоняет те же наборы по-русски. По умолчанию
они говорят по-английски. Английского дубля внутри кода нет намеренно — пропавший ключ должен
быть заметен, а не прикрыт.

| Набор | Нужна MO2 | Что проверяет |
|---|---|---|
| `test_sources.py` | нет | гигиену исходников: нет управляющих символов и путей к машине, всё компилируется, каждый модуль описан в обоих README, а в архив выкладки действительно попадают переводы |
...
**Занятый порт занимался молча.** `HTTPServer` объявляет `allow_reuse_address = 1`, и на Windows
второй мост вставал на уже слушаемый адрес без единой ошибки: писал в лог «поднят» и не получал
ни одного запроса. Теперь транспортный набор занимает порт и требует отказа.

**Ошибка в запросе была неотличима от поломки моста.** И то, и другое выходило пятисоткой
с трассировкой, и вызывающий не мог решить, чинить запрос или повторять его. Теперь `ValueError`
из проверки входа — это 400, а всё остальное — 500, и набор держит их врозь.

**Папка переводов уехала в архив пустой.** Когда `locale\` стал папкой на язык, обход в `pack.py`
остался на один уровень и взял ноль переводов — выложенный плагин говорил бы голыми ключами.
Незаметно до самой выкладки, поэтому `test_sources.py` теперь спрашивает сам `pack.py`, что он
собирается взять.
