...
| **EnvoyVoiceAdapter** — звук и модели | `adapter-voice\` | `Envoy Framework - Voice Adapter` | [adapter-voice/README.md](adapter-voice/README.md) |
...
- **Адаптер** знает обе стороны: внутрь игры говорит вызовом функции через контракт моста,
  наружу ходит к службе по HTTP. Он не знает ни одной модели по имени.
- **Мод-модель** не содержит кода вообще. Это один листок в известной папке, который говорит
  адаптеру: вот служба, вот её адрес, вот что она умеет, вот как её поднять.

Отсюда главное свойство: **адаптер можно выложить людям**. В нём нет ни одного пути,
ни одного порта и ни одного языка, верных только на машине автора, — всё это привозит
мод-модель. Смена модели не требует ни новой сборки адаптера, ни правки его настроек:
человек ставит другой мод.
