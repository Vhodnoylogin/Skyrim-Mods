...
Проверки лежат **вне** пакета плагина и не импортируются им: `tests\` — сосед папки `mo2ailink\`,
...
— от имени папки плагина, порт берётся из `MO2AILINK_PORT` (по умолчанию 8930).

`test_busy_live.py` запускает утилиту через MO2, чтобы та заняла менеджер. Имя задаётся
переменной `MO2AILINK_TEST_APP` (по умолчанию `TexGen`) и должно быть **зарегистрированным
