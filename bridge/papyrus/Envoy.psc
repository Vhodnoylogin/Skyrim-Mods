...
{In-game API of the Envoy bridge. Contract version 3 - the same number GetInterfaceVersion returns.
...
;                Envoy_Revoked: being revocable buys speed and costs duty.
...
;
; Where the lines come from: Interface\Translations\Envoy*_<language>.txt, UTF-16LE,
; one "$KEY<tab>text" line each. A mod puts its own file next to ours and names it
; Envoy<Something>_<language>.txt, and its keys appear here. An unknown key comes
...
; Envoy_Answer and Envoy_SpeechDone, which carry nothing but the request number.
...
; bridge answers with Envoy_Ping, and the script has to call Pong with the same
...
;
;   event "Envoy_SpeechDone", string = ok | interrupted | failed, number = speech id
;   event "Envoy_Answer",     string = the model's answer,        number = request id
;
