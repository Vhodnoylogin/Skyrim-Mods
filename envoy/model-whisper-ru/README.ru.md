...
Мод-модель игроку ничего не показывает, и переводить в ней нечего. Единственная строка,
доходящая до человеческого глаза, — поле `name` листка, которое адаптер пишет в свой
журнал; журнал же переводится на стороне адаптера.

Другое дело `language`: он говорит, на каком языке модель слушает, и подписчик, который
объявляет словарь фраз, обязан быть на том же. Поэтому тестовые подписчики берут свои
